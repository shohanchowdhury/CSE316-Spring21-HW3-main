...
const Subregion = (props) => {
    ...
    const parentRegionName = (props.activeList.name);
		...
		<div class="subRegionViewer">
            {
                ...
                />
                :
                <div className="table-text regionNamer"
                            onClick={() => toggleDescrEdit(!editingDescr)}
                            >{"Region Name:    " + description}
                            </div>
               
            }
            <div class = "subRegionParent">
                {"Parent Region:    " + parentRegionName}
            ...
            </div>
            {
                        editingDate  || due_date === ''
                            ...
                            wType="outlined" barAnimation="solid" inputClass="table-input-class"
                            />
                            : <div className="table-text regionDater"
                            onClick={() => toggleDateEdit(!editingDescr)}
                            >{"Region Capital:    " + (due_date.substring(0,due_date.lastIndexOf("//")))}
                            </div>
            }
            {
                editingStatus?
                        <input
                            ...
                            wType="outlined" barAnimation="solid" inputClass="table-input-class"
                        />
                            : <div className="table-text regionLeader"
                            onClick={() => toggleStatusEdit(!editingStatus)}
                            >{"Region Leader:    "+ due_date.substring(due_date.lastIndexOf("//")+2,due_date.length)}
                            </div>

                            
            }
		
                <span   className='closeRegion' onClick={(closingSubRegion)} style={{color:"white"}}>
                <i className="material-icons addTable">close</i>
                </span>
