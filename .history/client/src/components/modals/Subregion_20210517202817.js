...
const Subregion = (props) => {
    ...
    const handleStatusEdit = (e) => {
        ...
        const newDate = e.target.value ? e.target.value : '';
        const prevDate = due_date;
        let setterDate = due_date + newDate
        setDue_date(n)
		...
		<div class="subRegionViewer">
            ...
            }
            {
                        editingDate  || due_date === ''
                            ...
                            autoFocus={true} defaultValue={ due_date.substring(due_date.substring(0,due_date.lastIndexOf("//"))) } type='text'
                            wType="outlined" barAnimation="solid" inputClass="table-input-class"
                            />
                            : <div className="table-text"
            ...
            }
            {
                editingStatus?
                        <input
                            ...
                            autoFocus={true} defaultValue={  due_date.substring(due_date.substring(due_date.lastIndexOf("//"),due_date.length))   } type='text'
                            wType="outlined" barAnimation="solid" inputClass="table-input-class"
                        />
                            : <div onClick={() => toggleStatusEdit(!editingStatus)} className={` table-text`}>
                                { due_date.substring(due_date.substring(0,due_date.lastIndexOf("//"))) }
                            </div>
